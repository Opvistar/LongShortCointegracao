...
# DATABASE PYTHON NAO ESQUECER DE CONFIGURAR QUAIS
# ATIVOS MIGRAR NO LISTA_B3 em QuantMT5ConstantesGlobais.py
############################################################################################

print(" ---------- Versão do Script de Migração Única: V. - 1.2 24.12.2021 ----------")
...
for ativo in CTE.LISTA_B3:
    ...
    # diario
    str_path_tryd = os.path.join(os.getcwd(), CTE.TRYD, ativo + "_tryd.csv")
    
    #diario
    str_path_py = os.path.join(os.getcwd(), CTE.PYTHON, ativo + "_python.csv")
