...
#  -- 24.12.2021 -- REMOVENDO PATHS ABSOLUTOS NO CÓDIGO.

############################################################################################
############################################################################################


str_path_log = os.path.join(os.getcwd(), CTE.MIGRACAO +"Log_migracao_diaria_" + 
                            datetime.datetime.now().strftime('%Y_%d_%m_%H_%M_%S') + ".txt" )


file_relat = open(str_path_log, "a")  # DEBUG

ultima_data = datetime.datetime.now().strftime('%d/%m/%Y') 

#diario
str_path_py =  os.path.join(os.getcwd(), CTE.PYTHON)


print(" ---------- Versão do Script de Migração Diária: V. - 1.2  24.12.2021 ----------")
...
try:
    ...
    fechamento_dia_hoje = pd.read_csv("LISTA_ATIVOS_B3_FECHAMENTO.csv", sep=';', engine='python' , encoding = "ISO-8859-1")
